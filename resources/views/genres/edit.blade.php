...
@section('content')
    <h1>Welcome to Update genre =  {{ $update->name_genre}}</h1>
...
<form class="max-w-md mx-auto" action="{{ route('genre.update', $update->id )}}" method="POST">
  ...
  <div class="relative z-0 w-full mb-5 group">
      <input type="text" value="{{ $update->name_genre}}" name="name_genre"class="block py-2.5 px-0 w-full text-sm text-headin g bg-transparent border-0 border-b-2 border-default-medium appearance-none focus:outline-none focus:ring-0 focus:border-brand peer" placeholder="  " required />
